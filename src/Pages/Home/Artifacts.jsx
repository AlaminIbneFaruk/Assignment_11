...
const Artifacts = () => {
  ...
  py-6 uppercase tracking-wider">Explore Unique Artifacts</div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 text-base lg:text-xl lg:m-20">{Artifacts.map(artifact=><Artifact artifact={artifact} key={artifact._id} >{artifact.artifactName}</Artifact>)}</div>
      <div>
        <button
        onClick={handleView} 
